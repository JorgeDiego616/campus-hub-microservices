...
exports.LATEST_FEDERATION_VERSION = exports.LINK_SPEC_IDENTITY = exports.FEDERATION_SPEC_IDENTITY = void 0;
exports.versionCode = versionCode;
exports.printVersion = printVersion;
exports.federationSpecDocument = federationSpecDocument;
exports.linkSpecDocument = linkSpecDocument;
exports.federation1SpecDocument = federation1SpecDocument;
exports.importableNames = importableNames;
/**
...
exports.FEDERATION_SPEC_IDENTITY = 'https://specs.apollo.dev/federation';
exports.LINK_SPEC_IDENTITY = 'https://specs.apollo.dev/link';
/** Encodes a version as a single comparable integer (2.10 -> 210). */
function versionCode({ major, minor }) {
    return major * 100 + minor;
}
function printVersion({ major, minor }) {
    return `v${major}.${minor}`;
}
/** The most recent federation version this library knows how to link against. */
exports.LATEST_FEDERATION_VERSION = { major: 2, minor: 15 };
const version = (major, minor) => versionCode({ major, minor });
const TAG_LOCATIONS = 'FIELD_DEFINITION | INTERFACE | OBJECT | UNION | ARGUMENT_DEFINITION | SCALAR | ENUM | ENUM_VALUE | INPUT_OBJECT | INPUT_FIELD_DEFINITION';
const SCOPED_LOCATIONS = 'FIELD_DEFINITION | OBJECT | INTERFACE | SCALAR | ENUM';
/**
...
`;
function documentFrom(sdl) {
    return (0, graphql_1.parse)(sdl, { noLocation: true });
}
const federationDocumentCache = new Map();
/**
 ...
 */
function federationSpecDocument(version) {
    const code = versionCode(version);
    const cached = federationDocumentCache.get(code);
    if (cached)
        return cached;
    const sdl = FEDERATION_DEFINITIONS.filter((definition) => definition.since <= code && (definition.until ?? Infinity) >= code)
...
let linkDocument;
function linkSpecDocument() {
    return (linkDocument ??= documentFrom(LINK_SPEC_SDL));
}
let federation1Document;
function federation1SpecDocument() {
 ...
 */
function importableNames(version) {
    const names = new Set();
